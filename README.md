...
|---------|----------|-------|
| `/ac status <ник>` | показать пинг/TPS цели | `ac.command.status` |
...
| `/ac inspect <ник>` | dev/ops‑инструмент просмотра метрик | `ac.alerts` |

### Права
| Право | Назначение |
|-------|------------|
| `ac.use` | базовый доступ к `/ac` (по умолчанию: OP) |
| `ac.alerts` | получать алёрты и использовать `/ac inspect` |
| `ac.command.status` | использование `/ac status` |
