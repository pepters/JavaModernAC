...
public class AlertEngine {
    ...
    public void reload() {
        ...
        plugin.getLogger().info("AlertEngine: started (delay=" + delayMin + "-" + delayMax + "s, batch=" + batchWindow + "s, rate=" + rateLimit + "s)");
    }
    ...
    private void sendDebug(UUID target, AlertDetail detail) {
        ...
        String msg = mm.getMessage("alerts.debug_format")
    ...
    private void flush() {
        ...
        MessageManager mm = plugin.getMessageManager();
        for (Iterator<Map.Entry<UUID, List<AlertDetail>>> it = queues.entrySet().iterator(); it.hasNext(); ) {
            ...
            synchronized (list) {
                ...
                String msg = mm.getMessage("alerts.staff_format")
